...
<style>
    ...
    .stApp { background-color: #0A1F0F; }
    section[data-testid="stSidebar"] { background-color: #0D2613; border-right: 1px solid #1A3D24; }
    section[data-testid="stSidebar"] * { color: #E0E0E0 !important; }

    .main-title {
        font-family: 'Inter', sans-serif;
        font-size: 2.2rem; font-weight: 700;
        color: #22C55E; margin-bottom: 0.2rem;
    ...
    .section-title {
        ...
        color: #F0F0F0; margin: 1.5rem 0 0.5rem 0;
    ...
    .kpi-card {
        background: #132E19; border: 1px solid #22C55E;
        border-radius: 10px; padding: 1rem 1.2rem;
        text-align: center;
    }
    .kpi-label { font-size: 0.72rem; color: #86EFAC; text-transform: uppercase; letter-spacing: 1px; }
    .kpi-value { font-size: 2rem; font-weight: 700; color: #22C55E; line-height: 1.1; }
    .kpi-sub { font-size: 0.75rem; color: #9CA3AF; margin-top: 0.2rem; }
    hr { border-color: #1A3D24; }
    .stRadio > div { flex-direction: column; }
    div[data-testid="stMetric"] { background: #132E19; border-radius: 8px; padding: 0.8rem; }
</style>
...
# ── HELPER ────────────────────────────────────────────────────────────────────
def chart_header(titolo, descrizione='', fonte=''):
    st.markdown(f'<div class="chart-title">{titolo}</div>', unsafe_allow_html=True)
    if descrizione:
        st.markdown(f'<div class="chart-desc">{descrizione}</div>', unsafe_allow_html=True)

# ── SIDEBAR ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## ⚡ L-8 Ingegneria")
    st.markdown("### dell'Informazione")
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown('<span style="font-size:0.75rem;color:#6B7280;">Dati: ANVUR · MUR-USTAT · AlmaLaurea<br>A.A. 2020–2025</span>', unsafe_allow_html=True)

sezione = sezione.split(" ", 1)[1]
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    fig_avvi.add_trace(go.Scatter(
        ...
        name=f'Media: {media_avvi:,.0f}', hoverinfo='skip'))
    fig_avvi.add_trace(go.Bar(
        ...
        textposition='outside', textfont=dict(color='#F0F0F0', size=14, family='Inter'),
        hovertemplate='<b>%{x}</b><br>Avvii: <b>%{y:,.0f}</b><extra></extra>',
    ...
    for i, row in avvi_naz.iterrows():
        ...
        fig_avvi.add_annotation(x=row['anno'], y=row['avvii'] * 0.5,
            text=f"{simbolo} {abs(var):.1f}%",
            showarrow=False, font=dict(size=13, color=colore, family='Inter'))
    fig_avvi.update_layout(
        font=dict(family='Inter', size=13), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
    ...
    fig_avvi.update_xaxes(showgrid=False, tickfont=dict(color='#E0E0E0', size=14),
        linecolor='#2D5A3D', tickmode='linear', dtick=1)
    fig_avvi.update_yaxes(gridcolor='#1A3D24', tickfont=dict(color='#E0E0E0'),
        linecolor='#2D5A3D', rangemode='tozero',
        title=dict(text='N° avvii di carriera', font=dict(color='#E0E0E0')),
    ...
    buttons_tree = [dict(label=str(a), method='update',
        args=[{'visible': [j==i for j in range(len(anni_tree))]},
              {'title': dict(text=f'Avvii per Famiglia L-8 — {a}',
               font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center')}])
        for i, a in enumerate(anni_tree)]
    fig_tree.update_layout(
        title=dict(text=f'Avvii per Famiglia L-8 — {anni_tree[0]}',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        ...
        height=560, margin=dict(t=120, b=40, l=20, r=20),
    ...
    for i, anno in enumerate(anni_map):
        ...
        buttons_map.append(dict(label=str(anno), method='update',
            args=[{'visible': vis}, {'title': dict(text=f'Avvii L-8 per Regione — {anno}',
                font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center')}]))
    fig_map.update_layout(
        title=dict(text=f'Avvii L-8 per Regione — {anni_map[0]}',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        ...
        annotations=[
            dict(x=0.01, y=-0.02, xref='paper', yref='paper',
                text="Grigio: Valle d'Aosta, Molise, Basilicata — nessun corso attivo",
                showarrow=False, font=dict(size=10, color='#B0B0B0'), align='left'),
            dict(x=0.99, y=-0.02, xref='paper', yref='paper',
                text='Fonte: ANVUR — iC00a', showarrow=False,
                font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        margin=dict(r=20, t=110, l=0, b=40), height=620,
    ...
    for i, anno in enumerate(anni_top):
        subset = (g_top15[g_top15['Anno accademico']==anno]
            .sort_values('Numeratore', ascending=False).head(15)
            .sort_values('Numeratore', ascending=True).reset_index(drop=True))
        fig_top.add_trace(go.Bar(
            x=subset['Numeratore'], y=subset['Ateneo'], orientation='h',
            marker=dict(color=[COLORI_MACRO.get(m,'#6B7280') for m in subset['macro']],
    ...
    for macro, colore in COLORI_MACRO.items():
        fig_top.add_trace(go.Bar(x=[None], y=[None], orientation='h',
            marker_color=colore, name=macro, visible=True))
    buttons_top = [dict(label=str(a), method='update',
        args=[{'visible': [j==i for j in range(len(anni_top))]+[True]*len(COLORI_MACRO)},
              {'title': dict(text=f'Top 15 Atenei — L-8 {a}',
               font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center')}])
        for i, a in enumerate(anni_top)]
    fig_top.update_layout(
        font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text=f'Top 15 Atenei — L-8 {anni_top[0]}',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        updatemenus=[dict(type='buttons', direction='right', x=0.5, xanchor='center', y=1.10, yanchor='top',
            buttons=buttons_top, bgcolor=BG_CARD, bordercolor=VERDE_MAIN, borderwidth=1,
            font=dict(size=12, family='Inter', color='#F0F0F0'), active=0, pad=dict(r=6,l=6,t=6,b=6))],
        legend=dict(title=dict(text='Macro area', font=dict(color='#E0E0E0')),
            font=dict(color='#E0E0E0'), bgcolor='rgba(0,0,0,0)', x=0.78, y=0.05),
        margin=dict(t=120, b=60, l=220, r=120), height=560, barmode='overlay')
    fig_top.update_xaxes(title=dict(text='N° avvii', font=dict(color='#E0E0E0')),
        showgrid=False, tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D')
    ...
    fig_lazio.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='Avvii L-8 nel Lazio — Tradizionali vs Telematiche',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        annotations=[
            dict(x=0.22, y=1.10, xref='paper', yref='paper', text='<b>Atenei Tradizionali</b>',
                showarrow=False, font=dict(size=14, color='#86EFAC', family='Inter'), xanchor='center'),
            dict(x=0.78, y=1.10, xref='paper', yref='paper', text='<b>Atenei Telematici</b>',
                showarrow=False, font=dict(size=14, color='#FCD34D', family='Inter'), xanchor='center'),
            dict(x=0.99, y=-0.12, xref='paper', yref='paper', text='Fonte: ANVUR — iC00a',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        legend=dict(font=dict(color='#E0E0E0', size=11), bgcolor='rgba(0,0,0,0)',
            groupclick='toggleitem', x=1.01, y=1, xanchor='left', yanchor='top'),
    ...
    for macro in ['Nord','Centro','Sud','Isole']:
        ...
        fig_bubble.add_trace(go.Scatter(
            ...
            marker=dict(size=subset['Numeratore']/macro_pct['Numeratore'].max()*80+20,
                color=COLORI_MACRO[macro], opacity=0.85, line=dict(color='#0A1F0F', width=2)),
    ...
    fig_bubble.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='Quota Avvii per Macro Area — L-8 (2020–2025)',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        annotations=[dict(x=0.99, y=-0.10, xref='paper', yref='paper',
            text='Fonte: ANVUR — Dimensione bolla = avvii assoluti',
            showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        legend=dict(font=dict(color='#E0E0E0', size=12), bgcolor='rgba(0,0,0,0)', x=0.01, y=0.99),
        height=500, margin=dict(t=80, b=80, l=60, r=30))
    fig_bubble.update_xaxes(title=dict(text='Anno accademico', font=dict(color='#E0E0E0')),
        showgrid=False, tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D')
    fig_bubble.update_yaxes(title=dict(text='Quota (%)', font=dict(color='#E0E0E0')),
        gridcolor='#1A3D24', ticksuffix='%', tickfont=dict(color='#E0E0E0'),
        linecolor='#2D5A3D', range=[0, 70])
    ...
    for _, row in isc_naz.iterrows():
        fig_loll.add_shape(type='line', x0=row['anno_short'], x1=row['anno_short'],
            y0=0, y1=row['iscritti'], line=dict(color=VERDE_MAIN, width=3))
    fig_loll.add_trace(go.Scatter(
        x=isc_naz['anno_short'], y=isc_naz['iscritti'],
        mode='markers+text',
        marker=dict(size=20,
            color=[VERDE_LIGHT if a==isc_naz['anno_short'].iloc[-1] else VERDE_MAIN for a in isc_naz['anno_short']],
            line=dict(color='white', width=2)),
        text=isc_naz['iscritti'].apply(lambda x: f'{x:,.0f}'),
        textposition='top center', textfont=dict(color='#E0E0E0', size=12, family='Inter'),
    ...
    fig_loll.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text="Iscritti L-8 Ingegneria dell'Informazione — Italia (2019–2025)",
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
    ...
    fig_loll.update_yaxes(gridcolor='#1A3D24', tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D',
        rangemode='tozero', title=dict(text='N° iscritti', font=dict(color='#E0E0E0')),
        range=[0, isc_naz['iscritti'].max()*1.2])
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    indicatori_gauge = [
        ('pct_soddisfatti',  'Soddisfatti del corso', '#22C55E'),
        ('pct_riiscrizione', 'Si reiscriverebbero',   '#60A5FA'),
        ('pct_magistrale',   'Prosegue magistrale',    '#C084FC'),
    ]
    ...
    for anno_ref in anni_ref:
        ...
        for col_idx, (col, titolo, colore) in enumerate(indicatori_gauge):
            ...
            fig_gauge.add_trace(go.Indicator(
                mode='gauge+number+delta', value=val_2025[col],
    ...
    for i, anno_ref in enumerate(anni_ref):
        ...
        buttons_gauge.append(dict(label=f'vs {anno_ref}', method='update',
            args=[{'visible': vis}, {'title': dict(text=f'Profilo laureati L-8 — 2025 vs {anno_ref}',
                font=dict(size=20, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center')}]))
    fig_gauge.update_layout(
        title=dict(text='Profilo Laureati L-8 — 2025 vs 2024',
            font=dict(size=20, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        ...
        height=420, margin=dict(t=195, b=60, l=30, r=30),
        font=dict(family='Inter', size=12), paper_bgcolor=BG_PAPER,
        annotations=[
            ...
            dict(x=0.99, y=-0.18, xref='paper', yref='paper',
                text='Fonte: AlmaLaurea — Profilo dei Laureati L-8',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')])
    ...
    if alma_ok and not df_dest.empty:
        ...
        for col, colore in DEST_PLOT.items():
            if col == 'Altro':
                fig_dest.add_trace(go.Bar(x=df_dest2['anno'], y=df_dest2[col], name=col,
                    marker=dict(color=colore, line=dict(color='#0A1F0F', width=1), opacity=0.92),
        ...
        fig_dest.update_layout(barmode='stack', bargap=0.20,
            font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
            title=dict(text='Destinazione alla Magistrale — Laureati L-8 (2020–2025)',
                font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
            legend=dict(orientation='h', y=1.10, x=0.5, xanchor='center',
                bgcolor='rgba(0,0,0,0)', font=dict(color='#E0E0E0', size=12)),
            annotations=[
                dict(x=0.5, y=-0.20, xref='paper', yref='paper',
                    text=f"<b style='color:#22C55E'>{media_stesso:.1f}%</b>"
                         f"<span style='color:#9CA3AF'> resta nello stesso ateneo · </span>"
                         f"<b style='color:#60A5FA'>{media_nord:.1f}%</b>"
                         f"<span style='color:#9CA3AF'> sceglie un ateneo del Nord</span>",
                    showarrow=False, font=dict(size=12, family='Inter'), align='center'),
                dict(x=0.99, y=-0.26, xref='paper', yref='paper',
                    text="Fonte: AlmaLaurea · \"Altro\" include Sud-Isole, Telematico, Estero",
                    showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
            height=540, margin=dict(t=100, b=100, l=60, r=30))
        fig_dest.update_xaxes(title=dict(text='Anno di laurea', font=dict(color='#E0E0E0')),
            showgrid=False, tickfont=dict(color='#E0E0E0', size=14), linecolor='#2D5A3D')
        fig_dest.update_yaxes(title=dict(text='%', font=dict(color='#E0E0E0')),
            range=[0, 100], ticksuffix='%', gridcolor='#1A3D24',
            tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D')
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    fig_lau.add_trace(go.Bar(x=lau_naz['anno'], y=lau_naz['laureati'],
        ...
        textposition='outside', textfont=dict(size=10, color='#E0E0E0', family='Inter'),
    ...
    fig_lau.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        ...
        annotations=[
            dict(x=0.01, y=0.97, xref='paper', yref='paper',
                text=f"<b style='color:#22C55E'>+{crescita:.0f}%</b>"
                     f"<span style='color:#9CA3AF'> dal 2010 al 2024</span>",
                showarrow=False, font=dict(size=13, family='Inter'), align='left'),
            dict(x=0.99, y=-0.21, xref='paper', yref='paper',
                text='Fonte: MUR-USTAT — Laureati L-8 per anno solare',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        margin=dict(t=100, b=80, l=60, r=30), height=500)
    fig_lau.update_xaxes(showgrid=False, tickfont=dict(color='#E0E0E0', size=12),
        linecolor='#2D5A3D', tickmode='linear', dtick=1, tickangle=-45)
    fig_lau.update_yaxes(gridcolor='#1A3D24', tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D',
        rangemode='tozero', title=dict(text='N° laureati', font=dict(color='#E0E0E0')),
    ...
    for i, anno in enumerate(anni_donut):
        ...
        fig_donut.add_trace(go.Pie(
            ...
            hole=0.60, marker=dict(colors=['#22C55E','#F59E0B','#F87171'],
                line=dict(color='#0A1F0F', width=3)),
    ...
    for i, anno in enumerate(anni_donut):
        ...
        buttons_donut.append(dict(label=anno, method='update',
            args=[{'visible': vis}, {
                'title': dict(text=f"Cosa succede dopo il primo anno — L-8 {anno}",
                    font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
                'annotations': [
                    dict(text=f"<b>{row['prosegue_stesso']:.1f}%</b><br>"
                              f"<span style='color:#9CA3AF;font-size:11px'>resta nello<br>stesso corso</span>",
                         x=0.5, y=0.5, xref='paper', yref='paper', showarrow=False,
                         font=dict(size=24, color='white', family='Inter'), align='center'),
                    dict(x=0.99, y=-0.08, xref='paper', yref='paper',
                         text='Fonte: ANVUR iC14 + iC21 · Media nazionale corsi L-8',
                         showarrow=False, font=dict(size=10, color='#B0B0B0'), align='right', xanchor='right')]}]))
    row0 = df_destino[df_destino['anno']==anni_donut[0]].iloc[0]
    fig_donut.update_layout(
        title=dict(text=f"Cosa succede dopo il primo anno — L-8 {anni_donut[0]}",
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        updatemenus=[dict(type='buttons', direction='right', x=0.5, xanchor='center', y=1.12, yanchor='top',
            ...
            font=dict(size=12, family='Inter', color='#F0F0F0'), active=0, pad=dict(r=6,l=6,t=6,b=6))],
        annotations=[
            dict(text=f"<b>{row0['prosegue_stesso']:.1f}%</b><br>"
                      f"<span style='color:#9CA3AF;font-size:11px'>resta nello<br>stesso corso</span>",
                 x=0.5, y=0.5, xref='paper', yref='paper', showarrow=False,
                 font=dict(size=24, color='white', family='Inter'), align='center'),
            dict(x=0.99, y=-0.08, xref='paper', yref='paper',
                text='Fonte: ANVUR iC14 + iC21 · Media nazionale corsi L-8',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), align='right', xanchor='right')],
        height=560, margin=dict(t=120, b=80, l=80, r=80),
        font=dict(family='Inter', size=12), paper_bgcolor=BG_PAPER, showlegend=True,
        legend=dict(font=dict(color='#E0E0E0', size=12), bgcolor='rgba(0,0,0,0)',
    ...
    fig_ic02.add_trace(go.Scatter(
        x=[media_ic02, media_ic02],
        y=[str(int(ic02_naz['anno'].min())), str(int(ic02_naz['anno'].max()))],
        mode='lines', line=dict(color='#F59E0B', width=2, dash='dash'),
        name=f'Media: {media_ic02:.1f}%', hoverinfo='skip'))
    fig_ic02.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='% Laureati entro la Durata Normale — iC02 L-8',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        showlegend=True,
        legend=dict(font=dict(color='#E0E0E0', size=11), bgcolor='rgba(0,0,0,0)',
            orientation='h', x=0.5, xanchor='center', y=1.08),
        ...
        annotations=[
            dict(x=media_ic02, y=1.02, xref='x', yref='paper',
                text=f'Media: {media_ic02:.1f}%', showarrow=False,
                font=dict(size=11, color='#F59E0B'), align='center'),
            dict(x=0.99, y=-0.13, xref='paper', yref='paper',
                text='Fonte: ANVUR — % laureati entro durata normale (iC02) · Esclusi valori >100%',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')])
    fig_ic02.update_xaxes(showgrid=False, tickfont=dict(color='#E0E0E0'),
        linecolor='#2D5A3D', ticksuffix='%', range=[0, 50])
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    hover_multi = "<b>Multidisciplinare</b><br>Varianti incluse:<br>" + \
        "<br>".join([f"&nbsp;&nbsp;• {row['corso_pulito']}: <b>{row['pct']:.1f}%</b> ({row['n_at']} atenei)"
                     for _, row in multi_detail.iterrows()])
    ...
    def build_hover_var(row):
        ...
        return (f"<b>{row['famiglia']}</b><br>"
                f"Prosecuzione: <b>{row['prosecuzione']:.1f}%</b><br>"
                f"Abbandono: <b>{row['abbandono']:.1f}%</b><br>"
                f"N° atenei: <b>{row['n_atenei']}</b>")
    ...
    fig_var.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='Tasso di Prosecuzione per Famiglia di Corso — iC14 L-8',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        annotations=[
            dict(x=baseline, y=1.02, xref='x', yref='paper',
                text=f'Baseline Informatica: {baseline:.1f}%',
                showarrow=False, font=dict(size=10, color='#9CA3AF'), align='center'),
            dict(x=0.5, y=-0.20, xref='paper', yref='paper',
                text="<b style='color:#22C55E'>Verde</b> = sopra baseline · "
                     "<b style='color:#F87171'>Rosso</b> = sotto baseline · "
                     "Hover su Multidisciplinare per le varianti",
                showarrow=False, font=dict(size=11, color='#B0B0B0'), align='center'),
            dict(x=0.99, y=-0.26, xref='paper', yref='paper',
                text='Fonte: ANVUR iC14 · Media 2020–2024 per famiglia di corso',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        height=520, margin=dict(t=80, b=100, l=160, r=180))
    fig_var.update_xaxes(title=dict(text='% prosecuzione al II anno', font=dict(color='#E0E0E0')),
        showgrid=False, ticksuffix='%', tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D', range=[0, 100])
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    for macro in ['Nord','Centro','Sud']:
        ...
        fig_tasse_chart.add_trace(go.Bar(
            x=sub['Ateneo'], y=sub['Contributo max'], name=macro,
    ...
    for i, (_, row) in enumerate(non_statali_t.iterrows()):
        fig_tasse_chart.add_trace(go.Scatter(
    ...
    for i_t, card_t in enumerate(cards_t):
        ...
        shapes_t.append(dict(type='rect', xref='paper', yref='paper', x0=x0_t, x1=x1_t, y0=card_y0_t, y1=card_y1_t,
            fillcolor=BG_CARD_T, line=dict(color=card_t['colore'], width=2.5), layer='below', opacity=1))
        shapes_t.append(dict(type='rect', xref='paper', yref='paper', x0=x0_t, x1=x1_t, y0=card_y1_t-0.032, y1=card_y1_t,
            fillcolor=card_t['colore'], line=dict(width=0), layer='above', opacity=1))
        annotations_t.append(dict(x=cx_t, y=card_y1_t-0.02, xref='paper', yref='paper',
            text=f"<b>{card_t['titolo']}</b>", font=dict(size=13, color='white', family='Inter'),
            showarrow=False, xanchor='center', yanchor='middle'))
        annotations_t.append(dict(x=cx_t, y=cy_t+0.055, xref='paper', yref='paper',
            text=f"<b>{card_t['valore']}</b>", font=dict(size=36, color=card_t['colore'], family='Inter'),
            showarrow=False, xanchor='center', yanchor='middle'))
        annotations_t.append(dict(x=cx_t, y=cy_t-0.04, xref='paper', yref='paper',
            text=f"<b>{card_t['sub1']}</b>", font=dict(size=13, color='#F5F5F7', family='Inter'),
            showarrow=False, xanchor='center', yanchor='middle'))
        annotations_t.append(dict(x=cx_t, y=card_y0_t+0.025, xref='paper', yref='paper',
            text=card_t['sub2'], font=dict(size=12, color=C_TESTO2_T, family='Inter'),
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    for i, (anno, val) in enumerate(zip(anni_ic16, valori_ic16)):
        ...
        fig_radial.add_trace(go.Scatter(
            x=np.concatenate([x_outer, x_inner]), y=np.concatenate([y_outer, y_inner]),
            fill='toself', fillcolor=c, line=dict(color=c, width=0),
            name=f'{anno}: {val:.1f}%',
    ...
    fig_radial.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='Studenti al II Anno con ≥ 2/3 CFU — iC16BIS L-8',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        showlegend=True,
        legend=dict(font=dict(color='#E0E0E0', size=11), bgcolor='rgba(0,0,0,0)',
    ...
    for macro, colore in COLORI_MACRO_SCATTER.items():
        ...
        fig_corr.add_trace(go.Scatter(x=sub['imm_media'], y=sub['prosecuzione'],
    ...
    if len(df_tele) > 0:
        fig_corr.add_trace(go.Scatter(x=df_tele['imm_media'], y=df_tele['prosecuzione'],
            mode='markers', name='Telematiche',
            marker=dict(color='#F472B6', size=10, opacity=0.85, symbol='diamond',
                line=dict(color='#0A1F0F', width=1.5)),
            hovertemplate='<b>%{customdata}</b><br>Immatricolati medi: <b>%{x:.0f}</b><br>Prosecuzione: <b>%{y:.1f}%</b><extra></extra>',
    ...
    fig_corr.update_layout(font=dict(family='Inter', size=12), plot_bgcolor=BG_PLOT, paper_bgcolor=BG_PAPER,
        title=dict(text='Correlazione Dimensione Corso vs Tasso di Prosecuzione — L-8',
            font=dict(size=18, color='#F0F0F0', family='Inter'), x=0.5, xanchor='center'),
        annotations=[
            dict(x=0.98, y=0.98, xref='paper', yref='paper',
                ...
                align='right', xanchor='right', bgcolor=BG_CARD, bordercolor='#2D5A3D', borderwidth=1),
            dict(x=0.99, y=-0.10, xref='paper', yref='paper',
                text='Fonte: ANVUR iC00b + iC14 · Ogni punto = un corso · Media 2020–2024 · ◆ = Telematiche',
                showarrow=False, font=dict(size=10, color='#B0B0B0'), xanchor='right')],
        legend=dict(font=dict(color='#E0E0E0'), bgcolor='rgba(0,0,0,0)', x=0.01, y=0.01),
        height=540, margin=dict(t=80, b=80, l=70, r=30))
    fig_corr.update_xaxes(title=dict(text='Immatricolati medi per anno (2020–2024)', font=dict(color='#E0E0E0')),
        showgrid=True, gridcolor='#1A3D24', tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D', rangemode='tozero')
    fig_corr.update_yaxes(title=dict(text='Tasso prosecuzione al II anno (%)', font=dict(color='#E0E0E0')),
        showgrid=True, gridcolor='#1A3D24', ticksuffix='%', tickfont=dict(color='#E0E0E0'), linecolor='#2D5A3D')
